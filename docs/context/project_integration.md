...
### Post-A2 upstream — #82 Slice 2b roofFeature (commit `6f9a775`, pushed)
Quote-tool relay: additive optional `StrandItem.roofFeature?: RoofFeature | null` (`RoofFeature = gutter|peak|side|ridge|pathway|flat|metal`) — the PHYSICAL roof-attachment surface (distinct from `surface` the BILLING category); drives the quote tool's inventory clip-SKU selection. 2 files: `api.ts` (type + field) + `editor.ts` (4 byte-identical hunks: import; `isC9Roofline`/`roofFeatureOpts`/`sRoofFeature` block before the strand-panel `const sSurface`; a c9-only "Roof feature" `<select>` after the Surface dropdown; the `#sel-roof-feature` change handler). Harmless our side (Quote-binding gated off) but keeps cores in sync. tsc clean. Quote-tool-only (NOT mirrored): pricing, portal kind/icon, the purple satellite "Add Stake Run" tool, training fields.

### Post-A2 upstream — Stake Lighting surface (commit `c4206e9`, pushed)
Quote tool added a new independent lighting category "Stake Lighting" (sibling of Winter Wonderland, staked ground runs). ONE shared-core change mirrored: `Surface += "stake-lighting"` (api.ts c9 group) + the c9 `surfaceOpts` tuple in editor.ts gains `["stake-lighting", "Stake Lighting"]` as the 4th option. No new bulb type (it's a c9 surface tag). tsc clean; surfaceOpts byte-identical with their copy (flagged none differ). Everything else (pricing $6/7/8, portal kind, the purple `#a855f7` "Add Stake Run" satellite draw tool, training fields) is quote-tool-only — NOT mirrored.

### Post-A2 upstream — #63 draw-over (commit `28230bf`, pushed)
Quote-tool-authored 14-hunk diff to shared `pages/editor.ts`, mirrored verbatim (byte-identical). In draw mode with a click-drag "strand" tool (lights non-bistro, or garland), a gesture starting ON an existing item draws THROUGH it on a drag / selects it on a click. New symbols: `drawOverItemId`, `isStrandDrawContext()`, `selectDrawOverItem()`; each item group gets `setAttr("itemId")`; items render `draggable(false)` in strand-draw context (no drag arms); the 6 tool-change handlers `redrawScene()` to keep that flag fresh. tsc clean. NOT live-verified (Chrome DNS block).

### Post-A2 upstream — palette sync (commit `d2389df`)
Quote-tool relay: in `DEFAULT_COLORS`, relabel `cool-white` "Cool White"→"Pure White" (id/hex/glow unchanged) + add new builtin `cool-white-faceted` (label "Cool White", hex `#cee1e7`, glow `#eef5f8`). Palette now 12 builtins.
- **⚠️ TWO copies of `DEFAULT_COLORS` on our side — keep BOTH in lockstep on every palette change:** the shared core `client/src/editor/colors.ts` (byte-identical w/ quote tool) AND the **server seed** `server/src/routes/settings.ts`. The LIVE palette is served by the API (`/api/settings/colors` → server `DEFAULT_COLORS`, with backfill of missing builtins), so a colors.ts-only change has NO runtime effect — the server copy is what actually shows + backfills. (The client Settings page imports DEFAULT_COLORS from colors.ts, so it's covered by the core change; no 3rd copy.)
- Relabel caveat: an already-customized stored palette keeps the old `cool-white` label until Settings→Palette→Reset; the new faceted builtin backfills regardless. Not live-verified by me (Chrome DNS block); Jason can eyeball Settings→Palette.
