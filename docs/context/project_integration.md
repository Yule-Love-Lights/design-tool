...
- ⬜ **#4 STILL BROKEN — miniArea can't be selected / moved / resized.** Symptom: you CAN drag a placed box but it **snaps back** on release, and **no Transformer** (blue box) appears. The Rect-on-top hit fix (commit `fix(editor): make placed Scattershot/miniArea selectable`) did NOT fix it. By inspection the dispatch `click`/`dragend` handlers (editor.ts ~574 + ~600), the Transformer-attach (`transformer.nodes(selectedItemNodes)` ~631, miniArea falls to the default free-resize branch), and `bakeTransformIntoMiniArea` (box: `x:group.x(), y:group.y(), width/height`) all look correct — *parallel to the working spritzer*. So it's a **subtle runtime bug → debug LIVE in the browser console.** Place a miniArea, click it, and check: does the group's `click`/`dragend` fire? does `selectedIds` get the id? does `bakeTransformIntoMiniArea` run and persist `item.x/y`? is the miniArea node in `transformer.nodes()`? Likely suspects: the many `listening:false` bulb children vs the hit Rect; whether the group (not a child) receives the events; whether the post-select redraw recreates the group at the old position.
