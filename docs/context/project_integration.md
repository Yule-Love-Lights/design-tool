...
- **Design is now an independent record (quote side):** a design has its **own ID** + an **OPTIONAL** link to a quote (set when the quote is saved), NOT keyed to a quote. Supports "design before the quote exists" + the future standalone no-quote design-tool site. The locked contract's §3 storage shape (`designs(quote_id FK → quotes…)`) will be revised quote-side to match; **re-mirror `project_integration_data_contract.md` when they update it.**
