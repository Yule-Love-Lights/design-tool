...
**Next up:** **The AI Quote Tool integration is ACTIVELY UNDER BUILD** (Path B — absorb the editor into the quote tool; see [[project-integration]]). **A1** (gated quote-binding controls) and **A2** (mini-light area + railing grouping) shipped in Session 3; editor cores are kept **byte-identical** with the quote tool (they copy our `editor.ts` + `editor/miniArea.ts` verbatim). **NEXT = the A2 follow-up fixes:** #4 (Mini-Area selection bug — needs browser-console debug), #1/#3 (relocate to a Lights drag-to-draw "Scattershot" style + rename), #2 (railing price, quote side). **Full detail in [[project-integration]] "A2 follow-up fixes" + [[session-log]] Session 3 → NEXT.**
