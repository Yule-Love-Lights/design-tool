// SHARED EDITOR CORE — keep byte-identical with the quote tool (relay).
// Relayed from quote-tool #896 (ledger row 334).
//
...
//
// Lives in its own module (mirrors the quote tool's drawContext.ts precedent)
// rather than inside editor.ts because editor.ts imports Konva, which pulls in
// its Node entrypoint's optional `canvas` dependency outside a browser —
// keeping these functions Konva-free is what makes them unit-testable at all.
//
// RELAY NOTE — the two repos are NOT literally call-site-identical here, and
// that is deliberate, not drift: the quote tool has FOUR groupSelectedMini
// call sites (its #840 added mixed strand+scattershot groups) while this repo
// has TWO. The BEHAVIOUR relayed is the seed rule below plus the requirement
// that every call site keep passing its own historical fallback. Do not
// "harmonise" the fallbacks in either repo.
import type { StrandItem, MiniAreaItem } from "../api";
