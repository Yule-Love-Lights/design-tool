...
// #227 (quote-tool relay): a `miniGroup` (a grouped railing/curtain) whose member
// strands have ALL been deleted renders nothing (it has no points of its own —
// its extent is its members'), so the editor gives no way to select or delete
// it, yet the quote-tool's `projectScene` would otherwise keep emitting it and
// billing its `stringCount` forever. Call this immediately after ANY operation
// that removes strand items from a scene's `items` array, so the dangling group
// is deleted in the same edit instead of surviving as an unbillable-but-still-
